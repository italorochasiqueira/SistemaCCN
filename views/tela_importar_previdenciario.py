...
from controls.relatorios_gbe.importar_arquivos_bd import importar_arquivos_folha_bd
from controls.relatorios_gbe.importar_arquivos_bps import importar_arquivos_folha_bps
from controls.relatorios_gbe.importar_arquivos_pp import importar_arquivos_folha_pp

def abrir_tela_importar_previdenciario():
    janela = tk.Toplevel()
    ...
    lbl_titulo = tk.Label(frame_titulo, text="Importar arquivos Previdenciários", font=("Arial", 14, "bold"))
    ...
    def importar_arquivos():
        ...
        try:
            if plano == "BD":
                df = importar_arquivos_folha_bd(pasta)
            elif plano == "BPS":
                df = importar_arquivos_folha_bps(pasta)
            elif plano == "PostalPrev":
                df = importar_arquivos_folha_pp(pasta)
            else:
                df = None
